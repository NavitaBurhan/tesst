...
const GAMES_DATA = [
    ...
    // Battle Royale Games
    {
        ...
        denominations: [
            ...
            { amount: 12000, item: '120 Diamonds', popular: true },
            { amount: 25000, item: '250 Diamonds', popular: true },
            { amount: 50000, item: '500 Diamonds', popular: false },
            { amount: 100000, item: '1000 Diamonds', popular: false },
            { amount: 250000, item: '2500 Diamonds', popular: false }
        ]
    },
    {
        id: 'pubg-mobile',
...
function openGameModal(game) {
    ...
    document.getElementById('modalGameCategory').textContent = getCategoryName(game.category);
...
function createDenominationButton(denomination) {
    ...
    button.setAttribute('aria-label', `Pilih ${denomination.item} seharga ${formatCurrency(denomination.amount)}`);
    
    button.innerHTML = `
        <div class="item-name">${denomination.item}</div>
        <div class="item-price">${formatCurrency(denomination.amount)}</div>
    `;
    
    button.addEventListener('click', () => selectDenomination(denomination, button));
...
function updateOrderSummary() {
    ...
    document.getElementById('summaryItem').textContent = selectedDenomination.item;
    document.getElementById('summaryPlayerId').textContent = playerId || '-';
    document.getElementById('summaryPrice').textContent = formatCurrency(selectedDenomination.amount);
    document.getElementById('summaryTotal').textContent = formatCurrency(selectedDenomination.amount);
...
function validateForm() {
    ...
    const isValid = selectedDenomination && 
                   playerId.length > 0 && 
                   buyerName.length > 0 && 
                   buyerPhone.length >= 9;
